## data voor presentatie 20250402
packages <- c("RODBC",
              "lubridate",
              "RPostgres",
              "dplyr",
...
stat <- paste("select farms_id,lactation,dim,date_time,pedigree_id,production,ureum,fat_pct,cells from view_melkcontrole 
              where farms_id in (293)
              and date_time > '2021-01-01' 
              and lactation < 6
              and cells >0
              and dim < 401") # Friesian
df0 <- quer_pg(stat)
df <- df0 |> 
...
stat <- paste("select farms_id,lactation,dim,date_time,pedigree_id,production from view_productions 
              where farms_id in (293)
              and production>0
              and date_time > '2020-01-01'") # Friesian,Karp,KI
df2 <- quer_pg(stat)
df2 <- df2 |> 
  filter(!is.na(dim)&!is.na(production)) |> 
  filter(production>0) |> 
...
shinyApp(
  ui = fluidPage(
    sliderInput(
      'lactations',"Laktationen:",
      min = 1,max = max(df2$lactation),
      step = 1,
      value = c(1,3)
    ),
    sliderInput( 
      "dils", "Laktationstagen", 
  ...
  server = function(input, output) {
    output$plot = renderEcharts4r({
      df2 |> 
        filter(lactation>=input$lactations[1]&
                lactation<=input$lactations[2]&
                 dim>=input$dils[1]&
               dim<=input$dils[2]) |>
        group_by(date_time,nlactcat) |> 
        summarise(production = mean(production,na.rm = T)) |> 
        arrange(nlactcat,date_time)|> 
...
save(
  grafiek_productie,
  df2,file = 'beelden/20250414.Rdata')
